...
class MediaLabelNumbering(Document):
	def check_duplicate(self,prefix,new_value):
		new_name=prefix+cstr(new_value+1)
		select_doc_for_series='Media'
		media_ns = frappe.db.exists('Media', dict(
			name = new_name))
		if media_ns:
			frappe.throw(_("Series {0} already used in Media").format(new_name))
